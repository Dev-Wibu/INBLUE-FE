...
function SchedulePicker({ applicationDetailId, mentorId }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const localISOTime = new Date(joinTime.getTime() - tzOffset * 60 * 1000).toISOString();
  ...
  return (
    <Card title="Chọn lịch phỏng vấn với Mentor">
      ...
      <InputNumber value={duration} onChange={setDuration} min={15} max={180} addonAfter="phút" />
...
async function getMySessions(mentorId: number) {
  ...
  const upcoming = sessions.filter(
    (s) => s.userId2 === mentorId && ["SCHEDULED", "ONGOING"].includes(s.status)
  );

  const needReview = sessions.filter(
    (s) =>
      s.userId2 === mentorId && s.status === "COMPLETED" && (!s.mentorReview || !s.mentorFeedback)
  );
  ...
  return (
    <Card title={`Đánh giá ứng viên: ${studentName}`}>
      ...
      <TextArea
        ...
        onChange={(e) => setReview({ ...review, situationNote: e.target.value })}
      />
      {/* ... các field STAR khác */}
...
// 2. Xem vòng của mình
const details = await fetch(`/api/application-details/application/${applicationId}`, {
  headers: authHeader,
});
