...
- **Internationalization (i18n)**: All user-facing strings (labels, buttons, toasts, placeholders, validation messages) **must** use `t()` from `react-i18next`. Do not use hardcoded strings in `.tsx` files. When adding new keys, update `en.json`, `vi.json`, and `ja.json`. You **must** run `pnpm nx run inblue-fe:i18n:check-missing` or `pnpm validate` to ensure no keys are missing across all supported languages.
