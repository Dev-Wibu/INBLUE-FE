...
export function JobDescriptionRoundsDialog({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="flex h-[86vh] w-[94vw] max-w-6xl flex-col overflow-hidden border-slate-800 bg-slate-950 p-0">
        <DialogHeader className="border-b border-slate-800 bg-slate-900/50 px-6 pt-5 pb-4">
        ...
        {isLoading ? (
          ...
          <div className="flex min-h-0 flex-1 overflow-hidden bg-slate-950">
            {/* 1. LEFT SIDEBAR: Available Rounds Toolbox (30%) */}
            <div className="flex w-[28%] min-w-[280px] flex-col border-r border-slate-800 bg-slate-900/30">
              <div className="border-b border-slate-800 bg-slate-900/20 p-4">
            ...
            {/* 2. CENTER PANEL: Pipeline Workflow (44% - 72% depending on configuration sidebar) */}
            <div
              className={cn(
                "relative flex flex-1 flex-col overflow-hidden",
                // Grid background like n8n/node-editor
                    ...
                    return (
                      <div key={index} className="flex items-center gap-2">
                        ...
                        <div
                          onDragOver={(e) => e.preventDefault()}
                          onDrop={() => {
                            if (activeDragType) handleDropFromToolbox(activeDragType, index + 1);
            ...
            {selectedRoundIndex !== null && selectedRound && (
              <div className="animate-in slide-in-from-right flex w-[32%] min-w-[340px] flex-col overflow-hidden border-l border-slate-800 bg-slate-900/40 duration-250">
                ...
                <ScrollArea className="flex-1 p-5">
                  <div className="space-y-5 pb-6">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="space-y-1.5">
                        <Label className="text-xs font-semibold text-slate-300">
                          Điểm đạt yêu cầu (Pass Threshold)
                        </Label>
                        <div className="flex items-center gap-3">
                          <input
                            type="range"
                            min="0"
                            max="100"
                            value={Math.round((selectedRound.passThreshold ?? 0.8) * 100)}
                            onChange={(e) =>
                              updateRoundField(
                                selectedRoundIndex,
                                "passThreshold",
                            ...
                            className="accent-primary bg-slate-850 h-1.5 flex-1 rounded-lg"
                          />
                          <span className="w-10 text-right font-mono text-sm font-bold text-slate-200">
                      ...
                      {"timeLimitMinutes" in (selectedRound.configData || {}) && (
                        <div className="space-y-1.5">
                          <Label className="text-xs font-semibold text-slate-300">
                    ...
                    <hr className="my-4 border-slate-800/60" />
                    ...
                    <div className="space-y-4">
                      ...
                      {selectedRound.roundType === "CV_SCREENING" && (
                        <>
                          <div className="space-y-1.5">
                            ...
                            <Input
                              value={selectedRound.configData?.submissionFormat || ""}
                              onChange={(e) =>
                                updateRoundConfigField(
                                  selectedRoundIndex,
                                  "submissionFormat",
                      ...
                      {/* 2. QUIZ (QuizQuestions list builder) */}
                      {selectedRound.roundType === "QUIZ" && (
                        <div className="space-y-4">
                          <div className="flex items-center justify-between">
                            <Label className="text-primary text-xs font-bold tracking-wider text-slate-300 uppercase">
                              Danh sách câu hỏi (
                            ...
                            <Button
                              variant="outline"
                              size="sm"
                              className="h-7 border-slate-700 bg-slate-900 text-[11px] text-slate-300 hover:text-white"
                              onClick={() => {
                                ...
                                updateRoundConfigField(selectedRoundIndex, "quizQuestions", [
                                  ...currentQuestions,
                                  newQuestion,
                                ]);
                              }}>
                          ...
                          <div className="max-h-[40vh] space-y-3 overflow-y-auto pr-1">
                            {(selectedRound.configData?.quizQuestions || []).map((q, qIdx) => (
                              <div
                                key={qIdx}
                                className="group/question relative space-y-3 rounded-xl border border-slate-800/80 bg-slate-950/40 p-3">
                                <button
                                  onClick={() => {
                                    const currentQuestions =
                                      selectedRound.configData?.quizQuestions || [];
                                    const filtered = currentQuestions.filter((_, i) => i !== qIdx);
                                    updateRoundConfigField(
                                      selectedRoundIndex,
                                      "quizQuestions",
                                      filtered
                                  ...
                                  className="absolute top-2 right-2 text-slate-500 opacity-0 transition-opacity group-hover/question:opacity-100 hover:text-red-400"
                                  title="Xóa câu hỏi">
                            ...
                            {(!selectedRound.configData?.quizQuestions ||
                              selectedRound.configData.quizQuestions.length === 0) && (
                              <div className="border-slate-850 rounded-xl border border-dashed py-6 text-center text-xs text-slate-500">
