...
import { ChevronLeft, Edit3, ArrowLeft, Clock, FileText, CheckCircle } from "lucide-react";
...
export function JobDescriptionDetailView({
        ...
        configData: {
          ...
          codingProblems:
            r.configData?.codingProblemsId?.map((id) => {
              const cp = r.configData?.codingProblems?.find((problem: any) => problem.problemId === id);
              return {
                problemId: id,
  ...
  const selectedRoundMeta = selectedRound ? templates.find(t => t.type === selectedRound.roundType) : null;
  ...
  return (
    <div className="flex h-full flex-col bg-slate-50 dark:bg-slate-950">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="border-border/50 w-2/3 overflow-y-auto border-r p-6 relative">
          {selectedRound ? (
             <div className="mx-auto max-w-3xl space-y-8 animate-in fade-in slide-in-from-right-4 duration-300">
               <button 
                 ...
                 <div>
                   ...
                   <div className="mt-2 flex items-center gap-2">
                     <Badge variant="outline" className={cn("gap-1.5", selectedRoundMeta?.color)}>
                        {selectedRoundMeta?.icon && React.cloneElement(selectedRoundMeta.icon as React.ReactElement, { className: "h-3.5 w-3.5" })}
                        {selectedRoundMeta?.title}
                     </Badge>
                   </div>
                 </div>
                 <Button onClick={() => setIsEditorOpen(true)} className="gap-2 bg-indigo-600 hover:bg-indigo-700">
               ...
               <div className="grid grid-cols-2 gap-4 md:grid-cols-3">
                 <div className="rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900/50">
                   ...
                   <div className="mt-1 flex items-center gap-1.5 text-lg font-bold text-slate-900 dark:text-white">
                     ...
                     {selectedRound.configData?.timeLimitMinutes ? `${selectedRound.configData.timeLimitMinutes} Phút` : "Không giới hạn"}
                   </div>
                 </div>
                 <div className="rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900/50">
                   <div className="text-xs font-semibold uppercase tracking-wider text-slate-500">
                  ...
                  return (
                    <div
                      ...
                      className={cn(
                        ...
                        isSelected ? "scale-105" : "hover:scale-105 hover:opacity-90 opacity-70"
                      )}>
                      <div
                        className={cn(
                          "flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-white shadow md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 dark:border-slate-900 transition-colors",
                          meta?.bgColor,
                          meta?.color,
                      ...
                      <div className={cn(
                        "w-[calc(100%-4rem)] rounded-xl border p-4 shadow-sm md:w-[calc(50%-2.5rem)] transition-colors",
                        isSelected 
                          ? "border-indigo-500 bg-white dark:border-indigo-400 dark:bg-slate-900 shadow-md"
                          : "border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900"
                      )}>
                        <div className="mb-1 flex items-center justify-between">
                          <div className={cn("font-bold", isSelected ? "text-indigo-700 dark:text-indigo-400" : "text-slate-900 dark:text-slate-100")}>
                            {round.name}
                          </div>
                        </div>
                        <div className="text-xs text-slate-500 dark:text-slate-400">
                          {meta?.title} • {round.configData?.timeLimitMinutes ? `${round.configData.timeLimitMinutes}m` : "∞"}
                        </div>
                      </div>
                    </div>
                  );
