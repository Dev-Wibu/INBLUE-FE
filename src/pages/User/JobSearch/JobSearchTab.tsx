...
const LEVEL_COLORS: Record<string, string> = {
  INTERN: "bg-teal-100 text-teal-700 dark:bg-teal-500/10 dark:text-teal-400 dark:border-teal-500/20 border border-transparent",
  FRESHER: "bg-blue-100 text-blue-700 dark:bg-blue-500/10 dark:text-blue-400 dark:border-blue-500/20 border border-transparent",
  JUNIOR: "bg-indigo-100 text-indigo-700 dark:bg-indigo-500/10 dark:text-indigo-400 dark:border-indigo-500/20 border border-transparent",
  MIDDLE: "bg-purple-100 text-purple-700 dark:bg-purple-500/10 dark:text-purple-400 dark:border-purple-500/20 border border-transparent",
};

const STATUS_COLORS: Record<string, string> = {
  OPEN: "bg-emerald-100 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400 dark:border-emerald-500/20 border border-transparent",
  CLOSED: "bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-400 dark:border-slate-700 border border-transparent",
  DRAFT: "bg-orange-100 text-orange-700 dark:bg-orange-500/10 dark:text-orange-400 dark:border-orange-500/20 border border-transparent",
};
...
function JobCard({
  ...
  // formatCurrency likely appends the currency symbol (e.g. VNĐ)
  ...
  return (
    <div
      onClick={onClick}
      className="group relative flex cursor-pointer flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white p-5 transition-all duration-300 hover:-translate-y-1 hover:border-indigo-300 hover:shadow-xl dark:border-slate-800 dark:bg-slate-900 dark:hover:border-indigo-500/40 dark:hover:shadow-indigo-500/5">
      {/* Header: Avatar + Title + Badges */}
      <div className="flex items-start justify-between gap-4">
        <div className="flex h-12 w-12 shrink-0 items-center justify-center overflow-hidden rounded-xl border border-slate-100 bg-slate-50 text-sm font-bold text-indigo-600 dark:border-slate-700/50 dark:bg-slate-800 dark:text-indigo-400 shadow-sm">
          {logoUrl ? (
            <img
              src={logoUrl}
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-1 flex items-center gap-2">
            {job.level && (
              <Badge
                variant="secondary"
                className={`px-2 py-0.5 text-[10px] font-bold tracking-wider uppercase shadow-none ${LEVEL_COLORS[job.level] || "bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-400"}`}>
                {job.level}
              </Badge>
            )}
            <span className="truncate text-xs font-medium text-slate-500 dark:text-slate-400">
              {jobAny.companyName || t("common.unknownCompany", "Công ty ẩn danh")}
            </span>
          </div>
        </div>
        <div className="flex shrink-0 flex-col items-end gap-1 text-right">
          <div className="flex items-center gap-1.5 rounded-full bg-slate-50 px-2.5 py-1 text-xs font-semibold text-slate-500 dark:bg-slate-800/50 dark:text-slate-400">
      ...
      <div className="my-5 h-px w-full bg-slate-100 dark:bg-slate-800/80" />
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="flex items-center justify-between pl-1">
          <Badge
            variant="secondary"
            className={`shadow-none px-2 py-0.5 text-[11px] font-bold ${STATUS_COLORS[job.status || "OPEN"] || STATUS_COLORS.OPEN}`}>
            {job.status === "OPEN"
      ...
      <div className="mt-6 flex items-center gap-3">
        <Button
          variant="ghost"
          className="h-10 flex-1 rounded-xl text-xs font-semibold text-slate-600 hover:bg-slate-100 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700 dark:hover:text-white transition-colors">
          {t("enterpriseJobsearchpage.viewDetails", "Chi tiết")}
        </Button>
        <Button
          onClick={onApply}
          className="h-10 flex-1 rounded-xl bg-indigo-600 text-xs font-semibold text-white shadow-sm hover:bg-indigo-700 dark:bg-indigo-500/90 dark:text-white dark:hover:bg-indigo-500 transition-colors">
...
export function JobSearchTab() {
  ...
  return (
    <section className="flex h-full flex-col overflow-hidden bg-slate-50 dark:bg-slate-950">
      {/* Top Action Bar */}
        ...
        <form onSubmit={handleSearch} className="flex w-full items-center gap-3 md:max-w-2xl">
          <div className="relative flex-1">
            ...
            <Input
              ...
              className="h-10 border-slate-200 bg-slate-50/50 pl-10 pr-10 text-sm focus-visible:ring-indigo-500/20 dark:border-slate-800 dark:bg-slate-950 dark:placeholder:text-slate-500"
            />
            {searchQuery && (
              <button
                type="button"
          ...
          <Button
            type="submit"
            className="h-10 shrink-0 bg-indigo-600 px-6 text-sm font-semibold text-white shadow-sm hover:bg-indigo-700 dark:bg-indigo-500/90 dark:text-white dark:hover:bg-indigo-500 transition-colors">
              ...
              return (
                <button
                  ...
                  className={`rounded-full border px-4 py-1.5 text-[11px] font-bold tracking-wide transition-all ${
                    isActive
                      ? "border-indigo-600 bg-indigo-600 text-white shadow-sm dark:border-indigo-500/30 dark:bg-indigo-500/20 dark:text-indigo-300"
                      : "border-slate-200 bg-white text-slate-600 hover:border-indigo-300 hover:bg-indigo-50 hover:text-indigo-700 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-400 dark:hover:border-indigo-500/30 dark:hover:bg-indigo-500/10 dark:hover:text-indigo-300"
                  }`}>
