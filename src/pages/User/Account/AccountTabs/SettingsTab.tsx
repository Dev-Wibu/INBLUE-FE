  ...
  return (
    <div className="grid grid-cols-12 gap-6 items-start">
        ...
        <div className="col-span-12 lg:col-span-8 space-y-4">
            <Card className="border-slate-800 bg-[#1a1f2c] p-5 shadow-sm text-slate-100">
                <h3 className="text-base font-bold flex items-center gap-2 mb-4"><Eye className="h-4 w-4 text-[#6366f1]"/> Giao diện & Hiển thị</h3>

                <div className="grid grid-cols-2 gap-4">
                    <fieldset className="space-y-2">
                        <Label className="text-xs font-medium text-slate-400">Giao diện màu</Label>
                        <RadioGroup value={theme} onValueChange={(v) => { setTheme(v as Theme); applyTheme(v as Theme); }} className="grid grid-cols-3 gap-2">
                            {[{v: "light", l: "Sáng", i: Sun}, {v: "dark", l: "Tối", i: Moon}, {v: "system", l: "Hệ thống", i: Monitor}].map(o => (
                                <Label key={o.v} htmlFor={o.v} className={cn("flex flex-col items-center gap-1 border p-2 rounded-lg cursor-pointer transition-all", theme === o.v ? "border-[#6366f1] bg-[#2d3748]" : "border-slate-700 bg-[#252a3a]")}>
                    ...
                    <div className="space-y-4">
                        <fieldset className="space-y-2">
                            <Label className="text-xs font-medium text-slate-400">Cỡ chữ</Label>
                            <div className="flex border border-slate-700 rounded-lg p-0.5 bg-[#252a3a]">
                                {[{v: "small", l: "Nhỏ"}, {v: "default", l: "Mặc định"}, {v: "large", l: "Lớn"}].map(o => (
                                    <button key={o.v} onClick={() => setFontSize(o.v as FontSize)} className={cn("flex-1 px-2 py-1 text-[11px] rounded-md transition-all", fontSize === o.v ? "bg-[#6366f1] text-white" : "hover:bg-[#2d3748]")}>
                                        {o.l}
                                    </button>
                                ))}
                            </div>
                        ...
                        <fieldset className="space-y-2">
                            <Label className="text-xs font-medium text-slate-400">Ngôn ngữ</Label>
                            <select value={language} onChange={(e) => handleLanguageChange(e.target.value as Language)} className="w-full border border-slate-700 p-2 rounded-lg bg-[#252a3a] text-xs outline-none">
            ...
            <div className="grid grid-cols-2 gap-4">
                <Card className="border-slate-800 bg-[#1a1f2c] p-5 shadow-sm text-slate-100">
                    <h3 className="text-base font-bold flex items-center gap-2 mb-3"><Zap className="h-4 w-4 text-[#6366f1]"/> Năng suất</h3>
                    <RadioGroup value={sidebarBehavior} onValueChange={(v) => setSidebarBehavior(v as SidebarBehavior)} className="space-y-2">
                        {[{v: "always-open", l: "Luôn mở"}, {v: "auto-collapse", l: "Thu gọn"}].map(o => (
                            <Label key={o.v} htmlFor={o.v} className={cn("flex items-center gap-2 border p-2 rounded-lg cursor-pointer text-xs", sidebarBehavior === o.v ? "border-[#6366f1] bg-[#2d3748]" : "border-slate-700 bg-[#252a3a]")}>
                ...
                <Card className="border-slate-800 bg-[#1a1f2c] p-5 shadow-sm text-slate-100">
                    <h3 className="text-base font-bold flex items-center gap-2 mb-3"><Bell className="h-4 w-4 text-[#6366f1]"/> Thông báo</h3>
        ...
        <aside className="hidden lg:block lg:col-span-3">
            <div className="lg:sticky lg:top-4">
                <Card className="p-4 border-slate-700 bg-[#1a1f2c] text-slate-100 shadow-sm rounded-xl space-y-4">
