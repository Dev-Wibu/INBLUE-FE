...
export function CodingRunResult({
  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-white text-slate-800 dark:bg-slate-950 dark:text-slate-200">
      <button
        type="button"
        onClick={onToggle}
        className="flex h-10 shrink-0 items-center justify-between border-b border-slate-200 px-4 text-left hover:bg-slate-50 dark:border-slate-800 dark:hover:bg-slate-900/70">
        <div className="flex items-center gap-2">
          <Terminal className="h-4 w-4 text-slate-500 dark:text-slate-400" />
          <span className="text-xs font-semibold">{t("entryTestSession.runResult")}</span>
        </div>
        <div className="flex items-center gap-3">
          <span
            ...
            {t("entryTestSession.testsPassed", {
      ...
      {!collapsed && (
        <div className="min-h-0 flex-1 overflow-y-auto p-3">
          {result.errorMessage && (
            <p className="mb-3 rounded-md bg-rose-50 p-2 text-xs text-rose-700 dark:bg-rose-950/60 dark:text-rose-300">
              {result.errorMessage}
            </p>
          )}
                ...
                <div
                  ...
                  <div className="flex items-center justify-between">
                    <span className="flex items-center gap-2 text-xs font-semibold">
                      ...
                      {t("entryTestSession.testCase", { number: testCase.index + 1 })}
                    </span>
                  ...
                  <div className="mt-2 grid gap-2 text-[11px] sm:grid-cols-3">
                    <CodeValue label={t("entryTestSession.input")} value={testCase.input} />
                    <CodeValue
                      label={t("entryTestSession.expected")}
                      value={testCase.expectedOutput}
                    />
                    <CodeValue
                      label={t("entryTestSession.actual")}
...
function CodeValue({ label, value }: { label: string; value: string }) {
  return (
    <div>
      ...
      <pre className="mt-1 overflow-x-auto font-mono whitespace-pre-wrap text-slate-800 dark:text-slate-200">
        {value}
      </pre>
    </div>
  );
